...
const TrainerDashboard = () => {
    const { loading: trainerHookLoading } = useTrainerHook(); 
    ...
    // IMPORTANT: Conditional Rendering for Loading State
    if (trainerHookLoading) {
        return (
            <div className="flex justify-center items-center min-h-screen bg-zinc-950">
                <div className="w-16 h-16 border-4 border-dashed rounded-full animate-spin border-[#faba22] my-auto mx-auto"></div>
                <p className="text-[#faba22] ml-4 text-xl">Loading trainer dashboard...</p>
            </div>
        );
    }

    // Render the dashboard content only if not loading
    return (
